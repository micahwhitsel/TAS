 ...
 * Arguments:
 * 0: Object <OBJECT>
 * 1: Size Type <SIZE> ["none","alpha","bravo","charlie","ranger","medical","training","full"]
...
// make training selections
if (_companySelector == "training") then {
    // make ace interaction category
    [_object,"cScriptQuickSelection_Training","Training",_Icon,["ACE_MainActions","cScriptQuickSelectionMenu"]] call FUNC(addAceCategory);

    // make addAction topic
    if !(_companySelector == "full") then {
        if (_requireCompanyVariable) then {
            _object addAction ["Training Quick Selection", {}, nil, 1.5, true, true, "", QUOTE([ARR_2(_this,'Training')] call FUNC(hasCompanyVariable)), 5];
        } else {
            _object addAction ["Training Quick Selection", {}, nil, 1.5, true, true, "", "true", 5];
        };
    };

    // add loadouts
    [_object,"Fire Team Leader","CAV_SOI_STUD_TL","",['ACE_MainActions','cScriptQuickSelectionMenu','cScriptQuickSelection_Training'],_requireCompanyVariable,_addActionAllowedOnCompany] call FUNC(addQuickSelection);
    [_object,"Automatic Rifleman","CAV_SOI_STUD_AR","",['ACE_MainActions','cScriptQuickSelectionMenu','cScriptQuickSelection_Training'],_requireCompanyVariable,_addActionAllowedOnCompany] call FUNC(addQuickSelection);
    [_object,"Grenadier","CAV_SOI_STUD_GR","",['ACE_MainActions','cScriptQuickSelectionMenu','cScriptQuickSelection_Training'],_requireCompanyVariable,_addActionAllowedOnCompany] call FUNC(addQuickSelection);
    [_object,"Rifleman","CAV_SOI_STUD_RM","",['ACE_MainActions','cScriptQuickSelectionMenu','cScriptQuickSelection_Training'],_requireCompanyVariable,_addActionAllowedOnCompany] call FUNC(addQuickSelection);
    [_object,"Combat Life Saver","CAV_SOI_STUD_CLS","",['ACE_MainActions','cScriptQuickSelectionMenu','cScriptQuickSelection_Training'],_requireCompanyVariable,_addActionAllowedOnCompany] call FUNC(addQuickSelection);
};